...
const double EDGE_PROBABILITY =
    0.1; // Chance of a dependency between any two non-hub services
const int MAX_NON_HUB_WEIGHT = 10; // Max w(u,v) for u != g
const int SWEEP_W_MAX = 150;       // Max hub broadcast weight (W) to test
const int ASYNC_TRIALS =
    100; // Number of random schedules to average for async success

// As per the Coq definitions: Inductive State := Glory | Gnash.
enum class State { Glory, Gnash };
...
// Definition force_g (s:statef) : statef := fun u => if dec u g then Glory else
// s u.
...
// Definition rest_weight (v:V) : nat := sum_all (fun u => if dec u g then 0
// else w u v).
...
int main() {
  ...
  Graph graph = generate_random_graph(NUM_VERTICES, HUB_NODE, EDGE_PROBABILITY,
                                      MAX_NON_HUB_WEIGHT);

  // Calculate max_rest for this graph topology. This is a fixed property of the
  // graph. Definition max_rest : nat := list_max (map rest_weight nonhubs).
  int max_rest = 0;
  ...
  for (int W = 0; W <= SWEEP_W_MAX; ++W) {
    ...
    std::cout << W << "," << max_rest << "," << percent_glory_sync << ","
              << success_prob_async << std::endl;
  }
